...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === "POST") {
    ...
    try {
      const query = `INSERT INTO clinics (name, latitude, longitude, address, description)
      VALUES ( ?, ?, ?, ?, ?);`;
      const values: any[] = [name, latitude, longitude, address, description];
  ...
  if (req.method === "DELETE") {
    ...
    if (!id) {
      res.status(400).json({ message: "Clinic id not provided" });
    }
    try {
