...
export default async function handler(
  ...
  const query = `SELECT id, months, installment, total FROM paymentplans ORDER BY id DESC LIMIT 20 OFFSET ${
    20 * (page - 1)
  } ;`;
  ...
  const countQuery = `SELECT COUNT(*) AS results FROM \`paymentplans\` ;`;
