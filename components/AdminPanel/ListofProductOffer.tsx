...
const ListofProductOffer = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      return productCategory ? categoryMatchers.has(normalize(productCategory)) : false;
    });
  ...
  const handleUpload = async () => {
    ...
    if (!selectedProductId) {
      setErrorMessage("Please choose a product before uploading.");
      return;
    }
    ...
    if (!matchedProduct) {
      setErrorMessage("Please choose a product from the selected category.");
      return;
    }
    ...
    try {
      ...
      await axios.post(`${API_URL}/offer1`, formData);
  ...
  const handleUpdateFileChange = async (
    ...
    try {
      ...
      await axios.put(`${API_URL}/offer1/${updatingOfferId}`, formData);
      await fetchOffers();
    } catch (err) {
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            ...
            <p className={styles.modalHint}>
              Choose a category first, then pick a product from that category.
            </p>
            ...
            <select
              ...
              disabled={!selectedCategoryId || filteredProducts.length === 0}
            >
              <option value="">
                {!selectedCategoryId
            ...
            {selectedCategoryId && filteredProducts.length > 0 && (
              <p className={styles.modalHint}>
                ...
                {filteredProducts.length === 1 ? "" : "s"} found in this category.
              </p>
            )}
      ...
      {offers.map((offer) => (
          <div key={offer._id} className={styles.offerCard}>
            ...
            <div className={styles.offerMeta}>
              <p>
                <strong>Product Name:</strong> {getProductLabel(offer)}
              </p>
              <p>
