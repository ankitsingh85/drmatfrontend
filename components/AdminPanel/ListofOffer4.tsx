"use client";
// Synced: Offer 4 is wired to backend /offer4 APIs.
...
const ListofOffer4 = () => {
  ...
  const fetchOffers = async () => {
    try {
      const res = await axios.get(`${API_URL}/offer4`);
      setOffers(res.data);
    } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    for (const preview of validFiles) {
      try {
        await axios.post(`${API_URL}/offer4`, {
  ...
  const handleDelete = async (id: string) => {
    try {
      await axios.delete(`${API_URL}/offer4/${id}`);
      fetchOffers();
    } catch (err) {
  ...
  const handleUpdate = (id: string) => {
    ...
    axios
      .put(`${API_URL}/offer4/${id}`, { imageBase64: file })
      .then(() => fetchOffers())
